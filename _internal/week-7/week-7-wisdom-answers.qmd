...
<!-- DK: Read latest version of make_p_table() instructions in TODO.txt in primer.tutorials. -->

<!-- DK: Add spanner for "Units" and "Treatment". -->

<!-- DK: Change Treatment variable to something like "Election Result" -->

<!-- DK: Use backticks to make variable names with spaces, so we don't need to use col_label(). -->

<!-- DK: Experiment with tribble for entering data. -->

<!-- AT: For scneario 1 we only ocus on candidates who have won. Doesn't that mean that the treatment cannot be (Win/Lose) -->

<!-- DK: First Preceptor Table (and Population Table) is predictive, so only "Outcome". -->

<!-- DK: The treatment variable from the causal model is always included among the Covariates in the corresponding predictive model. -->
...
tibble(
  ...
  `Win Margin` = c(2.5, -3.2, "...", 5.8, 1.7, "...", -4.5),
  ...
  tab_spanner(
    label = "Covariate",
    columns = c(`Win Margin`, age, party)
  ) |> 
  tab_footnote(
...
<!-- DK: Quote about using July ` for missing birth dates. -->
...
Counter-example to the assumption of unconfoundedness:
  ...
  DK: Add more to this by tying it closely to the definition.

<!-- Change question to all one paragraph which lists the four assumptions and asks for their definitions and one counter-example for each. -->
