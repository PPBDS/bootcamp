...
tibble(
  ...
  gt() |>
  tab_header(title = "Population Table: All Years and Locations") |>
  cols_label(
    source = md("Source"),
    year = md("Year"),
...
ggplot(pred_data2, aes(x = kind, y = estimate, fill = race)) +
  geom_col(position = position_dodge(width = 0.7), width = 0.6) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high),
                position = position_dodge(width = 0.7), width = 0.25) +
  labs(
    title = "Predicted 4th Grade Math Scores by Class Type and Race",
    subtitle = "Class type has almost no meaningful effect on math scores.",
    x = "Class Type",
    y = "Predicted Math Score",
    fill = "Race",
    caption = "Predicted math scores are based on data from the Tennessee STAR experiment. Error bars indicate 95% confidence intervals."
  ) +
  coord_cartesian(ylim = c(680, 765)) +
  theme_minimal(base_size = 14) +
  theme(
