...
Consider using `predictions()` to generate the predicted scores for new scenarios and use `plot_predictions()` to plot the effect of class type on predicted scores. 

## TF Notes
**Scene 1**
Fork the repo. Render the Quarto document. Use Cmd/Ctrl + Enter to load everything in the Console.
...
Create an “interesting” plot using three predictors:


```{r}
# Predict scores for different combinations of class type, race, and previous reading score
plot_predictions(fit_cs, by = c("kind", "race", "g4reading"))
```

Summary Paragraph Example:

The model estimates how much, on average, a student’s 4th grade math score would increase or decrease if they were in a small class rather than a regular-sized class, after accounting for race, years in small classes, and prior reading performance. The marginaleffects package allows us to explore these differences across various subgroups and scenarios, helping answer questions about the expected impact of class size changes in different policy contexts.
