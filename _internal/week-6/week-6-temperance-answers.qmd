...
x <- read_csv("https://raw.githubusercontent.com/PPBDS/bootcamp/refs/heads/master/_internal/data/STAR.csv",
  ...
  mutate(kind = recode(classtype,
                      ...
                      `2` = "regular",
                      `3` = "regular with aid")) |> 
  # DK: Edit data to combine "others" and "hispanic" into just "others". Done
...
Fit a linear regression model in which math score (`g4math`) is the outcome, and the predictors are class type (`kind`), race, years spent in small classes (`yearssmall`), and all interactions between class type and race (`kind*race`). Fit this model to an object named `fit_cs`.
...
After fitting the model, run `fit_cs` and besure to display the confidence intervals. 
```{r}
fit_cs |> 
  tidy(conf.int = TRUE)
```
...
\begin{equation}
\begin{aligned}
\mathbb{E}[\text{g4math}_i] =\;& \beta_0 
+ \beta_1 \cdot \text{kind}_i 
+ \beta_2 \cdot \text{race}_i \\
&+ \beta_3 \cdot \text{yearssmall}_i 
+ \beta_4 \cdot (\text{kind}_i \times \text{race}_i)
\end{aligned}
\end{equation}
$$
...
Those default plots can look messy and hard to read. You can use `draw = FALSE` in your code to get the prediction data, then use AI to make a nice lookng plot. Paste your code in the R code chunk below. 

```{r}
pred_data <- plot_predictions(fit_cs, by = c("kind", "race"), draw = FALSE)
ggplot(pred_data, aes(x = kind, y = estimate, fill = race)) +
  geom_col(position = position_dodge(width = 0.7), width = 0.6) +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high), 
                position = position_dodge(width = 0.7), width = 0.2) +
  labs(
    title = "Predicted Math Scores by Class Type and Race",
    x = "Class Type",
...
```
We analyzed the Tennessee STAR dataset using a linear regression model to examine how class type, race, and years in small classes relate to fourth grade math scores. Our results show that class size (small, regular, or regular with aid) has little effect on math scores—predicted values and their confidence intervals are similar across groups. Race does matter, with white and “others” students scoring higher on average than Black students. Interaction terms showed no significant evidence that class size impacts races differently. Overall, class size reductions did not yield meaningful math score improvements in this analysis.
```

<!-- AT: Is there anything else I should add? Some students finished a little early last class. -->
