...
Preceptor Table: 
  ...
  * Covariates: class structure, sex, zip code, birth weight, family income, parent education, race 
    <!-- AT: Birth Weight? Maybe add school district or teacher experience-->
...
```{r}
augment(fit_cs, new_data = ndata, se_fit = TRUE)
```
...
```{r}
ndata <- tibble(kind = c("regular", "small"))
augment(fit_cs, new_data = ndata) |>
  ggplot(aes(x = kind, y = .pred, fill = kind)) +
    geom_col(width = 0.5, alpha = 0.7) +
    labs(
      title = "Predicted Mean Exam Score by Class Size",
      subtitle = "No clear difference in predicted means",
      x = "Classroom Size",
...
```{r}
tidy(fit_cs, conf.int = TRUE) |> 
  filter(term == "kindsmall") |>
  ggplot(aes(x = estimate, y = 0)) +
    geom_point(size = 4) +
    geom_errorbarh(aes(xmin = conf.low, xmax = conf.high), height = 0.15, size = 1) +
    labs(
      title = "Estimated Causal Effect of Fewer Students",
      subtitle = "No significant difference in mean scores",
      x = "Expected Difference in Exam Scores (Small - Regular)",
      y = "",
      caption = "Model: Linear regression; Data: Tennessee STAR"
    ) +
    theme_minimal() +
    theme(
...
<!-- AT: The plots look a lot cooler with brms because its a Bayesian model. -->
