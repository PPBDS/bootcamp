...
library(gt)
library(tidybayes)
library(brms) #AT: Most likely needs to be replaced by a different package. brms functions need to be replaced as well. 
#AT: Add library(broom)?
```
...
AT: Will there essentially be two versions? A TF version and a seperate student version?
...
<!-- AT: the chunk above takes a while to load (at least for me). Replacing with a new function from a different package could reduce the time.-->
...
<!-- AT: Skips scene 4. Also explain code better or add more text.  -->
