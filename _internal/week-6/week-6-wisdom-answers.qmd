...
tibble(
  ...
  tab_spanner(label = "Covariates", columns = c(title1, parent_college)) |>
  fmt_markdown(columns = everything()) |>
  cols_align(align = "center", columns = everything()) |>
  cols_align(align = "left", columns = c(ID)) |>
  tab_footnote(
...
tibble(
  ...
  gt() |>
  tab_header(title = "Population Table: Chicago Students' Observed Math Scores") |>
  cols_label(
    ID = md("ID"),
    score_observed = md("Observed Math Score"),
    kind = md("Class Type"),
  ...
  tab_spanner(label = "Observed Data", columns = c(score_observed, kind, race, sex)) |>
  cols_align(align = "center", columns = everything()) |>
  cols_align(align = "left", columns = c(ID)) |>
  fmt_markdown(columns = everything())
...
Scenario 1 (Chicago): If you are only making predictions about current students (without considering interventions), unconfoundedness is less of a concern. The focus is on predicting observed performance given current characteristics, not estimating causal effects.
